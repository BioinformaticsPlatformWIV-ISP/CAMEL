...
from app.pipeline.yamlstep import YAMLStep
...
class YAMLPipeline(Pipeline):
    ...
    def __init__(self, yaml_files, camel, db_pipeline_parameters=False, db_logging=False):
        ...
        """
        super(YAMLPipeline, self).__init__(camel, db_logging)
        self._steps = []
        self._outputs = {}
        ...
        if db_pipeline_parameters is True or db_logging is True:
    ...
    def _parse_steps(self, step_data):
        ...
        for step in step_data:
            try:
                new_step = YAMLStep(step['id'], step['tool'](self._camel), self, self._camel)
                if 'inputs' in step:
    ...
    def _get_next_step(self, current_step):
        ...
        if current_step.next_step_specification is None:
            ...
            for branch in current_step.next_step_specification:
                if type(branch) is YAMLStep.ConditionalNextStep:
                ...
                elif type(branch) is YAMLStep.DefaultNextStep:
                    next_step_name = branch.step_id
                    break
                else:
    ...
    def _prepare_inputs_for_step(self, step):
        ...
        for input_ in step.input_specification:
            ...
            if type(input_) is YAMLStep.StepInput:
            ...
            elif type(input_) is YAMLStep.ExternalInput:
