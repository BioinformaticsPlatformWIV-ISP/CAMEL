...
class YAMLStep(Step):
    ...
    def __init__(self, name, tool, pipeline, camel):
        ...
        Initializes a step.
        """
        super(YAMLStep, self).__init__(name, tool, pipeline, camel)
        ...
        for item in self._input_specification:
            try:
                if 'external-input' in item:
                    ...
                    specification.append(YAMLStep.ExternalInput(files, item['alias']))
                else:
                    alias = item['alias'] if 'alias' in item else item['name']
                    required = item.get('required', False)
                    specification.append(YAMLStep.StepInput(item['name'], item['from'], alias, required))
    ...
    def inform_specification(self):
        ...
        for item in self._inform_specification:
            try:
                specification.append(YAMLStep.StepInform(item['from'], item['alias']))
            except KeyError as err:
    ...
    def next_step_specification(self):
        ...
        for branch in self._next_step_specification:
            try:
                if 'when' in branch:
                    specification.append(YAMLStep.ConditionalNextStep(branch['when']['condition'],
                                                                      branch['when']['step-id']))
                elif 'default' in branch:
                    specification.append(YAMLStep.DefaultNextStep(branch['default']['step-id']))
                else:
