...
class MothurCluster(Mothur):
    ...
    def _check_input(self):
        ...
        :return: None
        """
        super(MothurCluster, self)._check_input()
        if len([x for x in self._tool_inputs if x in ['FASTA', 'DIST', 'PHY']]) != 1:
            raise InvalidInputSpecificationError('Invalid input files (keys) given for Mothur cluster, only DIST or '
                                                 'PHY or FASTA allowed: {!r}'.format(self._tool_inputs))
        if 'TSV_Names' in self._tool_inputs and 'TSV_Counts' in self._tool_inputs:
            raise InvalidInputSpecificationError('Invalid input files (keys) given for Mothur cluster, TSV_Names and '
    ...
    def _set_output(self):
        ...
        # Only DIST or PHY is allowed, not both
        if 'DIST' in self._tool_inputs:
            basename = super(MothurCluster, self)._get_basename('DIST')
        elif 'PHY' in self._tool_inputs:
            basename = super(MothurCluster, self)._get_basename('PHY')
        elif 'FASTA' in self._tool_inputs:
            basename = super(MothurCluster, self)._get_basename()
