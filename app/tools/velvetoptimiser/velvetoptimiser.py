...
class VelvetOptimiser(Velvet):
    ...
    def __set_output(self):
        ...
        :return: none
        """
        self.output_dir = os.path.join(self._folder, self._parameters['output_dir'].value)
        self._tool_outputs['FASTA_Contig'] = [ToolIOFile(os.path.join(self.output_dir, self.OUTPUT_FASTA))]
    ...
    def __build_command(self):
        ...
        self._command.command = "{} {} -f {!r} -o {!r}".format(
            ...
            " ".join([self._input_string, self._parameters['velvethopts'].value]),
    ...
    def __set_inform(self):
        ...
        self.informs.update(
            VelvetgOutputAnalyzer.analyze_log_file(os.path.join(self.output_dir, self.LOG_FILE))
        )

        log_file_path = os.path.join(self.output_dir, self._parameters['log_prefix'].value + self.VO_LOG_FILE_SURFIX)
        ...
        # analyze the later
        self.informs.update(
            VelvetgOutputAnalyzer.analyze_stats_file(self._informs, os.path.join(self.output_dir, self.STATS_FILE))
        )
