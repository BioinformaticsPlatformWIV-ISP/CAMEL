...
class Bcftools(Tool, metaclass=abc.ABCMeta):
    """
    ...
    def _get_output_key(self) -> str:
        ...
        """
        if 'output_format' not in self._parameters:
            return 'VCF'
        try:
            return {
                'b': 'BCF_GZ',
                ...
                'z': 'VCF_GZ',
                'v': 'VCF'
            }[self._parameters['output_format'].value]
        except KeyError:
            raise InvalidParameterError(f"Invalid output format: {self._parameters['VCF']}")
