...
class Clair3(Tool):
    ...
    def _set_output(self) -> None:
        ...
        bcftools_view.add_input_files({'VCF': [ToolIOFile(path_out_fixed)]})
        bcftools_view.update_parameters(
            output_type='z', output_filename='clair3_variants-fixed.vcf.gz'
        )
    ...
    def __filter_problematic_positions(self, vcf_in: Path) -> Path:
        ...
        command = Command(
            f'module load bcftools/1.17; bcftools view {vcf_in} > {path_out_uncompressed};'
        )
        ...
        format_ok = True
        try:
            with cyvcf2.VCF(str(path_out_uncompressed)) as vcf_reader:
                _ = list(vcf_reader)
        except ValueError as err:
            format_ok = False
            logger.warning(f'Could not parse VCF generated by Clair3: {err}')

        # Parsing is OK -> return the original file
        if format_ok:
        ...
        with open(path_out_uncompressed) as handle:
            for line in handle.readlines():
                ...
                if not m:
                    ...
                    continue
                logger.warning(
                    f'Skipping variant at position {parts[1]} - incorrect format'
                )
