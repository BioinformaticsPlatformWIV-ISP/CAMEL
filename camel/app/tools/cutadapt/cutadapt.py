...
class Cutadapt(Tool):
    ...
    def _set_output(self) -> None:
        ...
        """
        if 'FASTQ_PE' in self._tool_inputs:
            self._tool_outputs['FASTQ'] = [ToolIOFile(self.folder / f"{self._parameters['output_basename'].value}_R1_trimmed.fastq.gz"),
                                           ToolIOFile(self.folder / f"{self._parameters['output_basename'].value}_R2_trimmed.fastq.gz")]
        elif 'FASTQ_SE' in self._tool_inputs:
            self._tool_outputs['FASTQ'] = [ToolIOFile(self.folder / f"{self._parameters['output_basename'].value}_trimmed.fastq.gz")]
