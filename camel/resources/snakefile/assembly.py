...
def get_fasta_raw(config: Dict[str, Any]) -> Path:
    ...
    """
    if (config['input_type'] in ('fasta', 'fasta_vcf')) and ('human_read_scrubbing' not in config['analyses']):
        return Path(str(human_read_scrubbing.INPUT_SCRUBBING_FASTA).format(input_format='fasta'))
    if (config['input_type'] in ('fasta', 'fasta_vcf')) and ('human_read_scrubbing' in config['analyses']):
...
def get_command_informs(config: Dict[str, Any]) -> List[Path]:
    ...
    """
    if config['input_type'] in ('fasta', 'fasta_vcf'):
        return []
    if config['input_type'] == 'illumina':
