...
rule polishing_polca:
    ...
    output:
        ...
        INFORMS = Path(config['working_dir']) / short_read_polishing.OUTPUT_ASSEMBLY_INFORMS
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'polca',
...
rule hybrid_assembly_filter_contig_length:
    ...
    output:
        FASTA = Path(config['working_dir']) / 'polishing' / 'filtering' / 'fasta.io',
        INFORMS = Path(config['working_dir']) / short_read_polishing.OUTPUT_ASSEMBLY_FILTERING_INFORMS
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'filtering',
...
rule hybrid_assembly_quast:
    ...
    output:
        TSV = Path(config['working_dir']) / 'polishing' / 'quast' / 'tsv.io'
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'quast'
    run:
...
rule hybrid_assembly_quast_extract_informs:
    ...
    output:
        INFORMS = Path(config['working_dir']) / 'polishing' / 'quast' / 'informs.io'
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'quast'
    run:
...
rule hybrid_assembly_report:
    ...
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'report',
...
rule hybrid_assembly_dump_summary_info:
    ...
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'summary'
    run:
...
rule hybrid_assembly_bt2_index:
    ...
    output:
        INDEX_GENOME_PREFIX = Path(config['working_dir']) / 'polishing' / 'bowtie2' / 'genome_prefix.io'
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'bowtie2'
    run:
...
rule hybrid_assembly_bt2_map:
    ...
    output:
        SAM = Path(config['working_dir']) / 'polishing' / 'bowtie2' / 'sam.io',
...
rule hybrid_assembly_bt2_sam_to_bam:
    ...
    output:
        BAM = Path(config['working_dir']) / 'polishing' / 'bowtie2' / 'bam.io'
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'bowtie2'
    run:
...
rule hybrid_assembly_bt2_sort_bam:
    ...
    output:
        BAM = Path(config['working_dir']) / 'polishing' / 'bowtie2' / 'bam_sorted.io'
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'bowtie2'
    run:
...
rule hybrid_assembly_samtools_depth:
    ...
    input:
        BAM = rules.hybrid_assembly_bt2_sam_to_bam.output.BAM
    output:
        TSV = Path(config['working_dir']) / 'polishing' / 'samtools_depth' / 'tsv.io',
        INFORMS = Path(config['working_dir']) / short_read_polishing.OUTPUT_ASSEMBLY_DEPTH_INFORMS
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'samtools_depth'
    run:
...
rule hybrid_assembly_nanopore_map_reads:
    ...
    input:
        ...
        FASTA = rules.hybrid_assembly_filter_contig_length.output.FASTA
    output:
        SAM = Path(config['working_dir']) / 'polishing' / 'minimap2' / 'sam.io',
        INFORMS = Path(config['working_dir']) / short_read_polishing.OUTPUT_ASSEMBLY_NANOPORE_MAPPING_INFORMS
    params:
        dir_ = Path(config['working_dir']) / 'polishing' / 'minimap2'
    run:
...
rule hybrid_assembly_nanopore_sam_to_bam:
    ...
    output:
        BAM = Path(config['working_dir']) / 'polishing' / 'minimap2' / 'bam.io'
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'minimap2'
    run:
        from camel.app.tools.samtools.samtoolsview import SamtoolsView
        samtools_view = SamtoolsView(camel)
        step = Step(str(rule), samtools_view, camel, params.running_dir)
...
rule hybrid_assembly_nanopore_sort_bam:
    ...
    output:
        BAM = Path(config['working_dir']) / 'polishing' / 'minimap2' / 'bam_sorted.io'
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'minimap2'
    run:
        from camel.app.tools.samtools.samtoolssort import SamtoolsSort
...
rule hybrid_assembly_nanopore_samtools_flagstat:
    ...
    params:
        dir_ = Path(config['working_dir']) / 'polishing' / 'samtools_flagstat'
    run:
...
rule hybrid_assembly_nanopore_samtools_depth:
    ...
    input:
        BAM = rules.hybrid_assembly_nanopore_sort_bam.output.BAM
    output:
        TSV = Path(config['working_dir']) / 'polishing' / 'samtools_depth_nanopore' / 'tsv.io',
