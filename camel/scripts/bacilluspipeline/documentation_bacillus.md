...
Coverage is estimated by dividing the total number of bases by the size of the `NC_000964.3` *B. subtilis* reference genome or the `NZ_CP017060.1` *B. cereus* reference genome. The total number of bases in the FASTQ files is determined using the `size` function of `seqtk 1.4`.
...
|----------------------------------------|-----------------------|--------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Kraken: contaminants                   | 1.00% / 5.00% (ONT)   | 5.00% / 10% (ONT)  | Percentage of reads / contigs assigned to genus other than *Bacillus*                                                                                                                                                           |
...
To determine the presence of a GMM in the sequencing data of *B. subtilis* samples, the pipeline determines the strain under study as well as the GMM construct. If the combination of a strain and GMM construct matches a previously described GMM, the pipelines determines the sample as GMM.

### GMM construct detection

The GMM construct detection in *B. subtilis* genomes is performed as described in [REF] using `kma 1.4.12a`. 

For completeness, the following GMM databases are available:

| **name**          | **description**                                      |
|-------------------|------------------------------------------------------|
| GMM junctions     | DNA sequences of unnatural junctions from known GMMs |
| Genes and vectors | Genes, plasmidic vectors and known GMM constructs    |

### GMM strain assignment

`StrainGST 1.3.9` is used with default parameters on the filtered reads using a custom database of 1,624 *Bacilli* genomes from RefSeq last downloaded on 9th of March 2024. 
...
| **name**     | **species**   | **origin**     |
|--------------|---------------|----------------|
| Classic MLST | *B. subtilis* | BIGsDB PubMLST |
| Classic MLST | *B. cereus*   | BIGsDB PubMLST |
| cgMLST       | *B. cereus*   | BIGsDB PubMLST |

## 12. *Bacillus cereus* characterization

`Btyper3 3.4.0` is used with default parameters on the sample if the species selected is *B. cereus*. This assay performs species identification by ANI, *B. cereus*-species virulence genes detection and sequence type assignment by MLST and `panC' sequence typing.
