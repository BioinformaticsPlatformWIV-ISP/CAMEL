...
rule assembly_flye_index_bam:
    ...
    input:
        BAM = rules.assembly_flye_sort_bam.output.BAM
    output:
        BAI = Path(config['working_dir']) / 'assembly_flye' / 'minimap2' / 'samtools_sort.bam.bai'
    params:
        running_dir = Path(config['working_dir']) / 'assembly_flye' / 'minimap2'
    run:
...
rule assembly_flye_medaka_consensus:
    ...
    output:
        HDF = Path(config['working_dir']) / 'assembly_flye' / 'medaka' / 'raw_hdf.io',
        INFORMS = Path(config['working_dir']) / 'assembly_flye' / 'medaka' / 'commands-consensus.io'
    params:
        running_dir = Path(config['working_dir']) / 'assembly_flye' / 'medaka',
...
rule assembly_flye_medaka_stitch:
    """
    ...
    input:
        ...
        FASTA = rules.assembly_flye_filter_contig_length.output.FASTA
    output:
        FASTA = Path(config['working_dir']) / 'assembly_flye' / 'medaka' / 'fasta.io',
        INFORMS = Path(config['working_dir']) / 'assembly_flye' / 'medaka' / 'commands-stitch.io'
    params:
        working_dir = Path(config['working_dir']) / 'assembly_flye' / 'medaka',
...
rule assembly_flye_quast:
    ...
    """
    input:
        FASTA = rules.assembly_flye_medaka_stitch.output.FASTA
    output:
...
rule assembly_flye_report:
    ...
    input:
        ...
        FASTA_Contig = rules.assembly_flye_medaka_stitch.output.FASTA,
