...
def qc_output_files(wildcards):
    """
    Create a dictionary of the expected qc output files. If the "--no-qc" flag is used, only the mark duplicates metrics
    file will be generated. This dictionary is the input of the move_qc rule
    """
    files = dict()
    if config["no_qc"]:
        files["mark_duplicates_metrics"] = Path(config['working_dir']) / 'qc' / 'mark_duplicates' / "duplicate_metrics.txt.io",
    else:
        files["TXT"] = expand(Path(config['working_dir']) / "qc" / "quality_yield" / "{fastq}.unmapped.quality_yield_metrics.io", fastq = config["input_basenames"]),

        files["base_distribution_by_cycle"] = expand(Path(config['working_dir']) / "qc" / "unsorted_RG_quality" / "{fastq}.unsorted_readgroup.base_distribution_by_cycle.pdf", fastq = config["input_basenames"]),
        files["base_distribution_by_cycle_metrics"] = expand(Path(config['working_dir']) / "qc" / "unsorted_RG_quality" / "{fastq}.unsorted_readgroup.base_distribution_by_cycle_metrics", fastq = config["input_basenames"]),
        files["insert_size_histogram_pdf"] = expand(Path(config['working_dir']) / "qc" / "unsorted_RG_quality" / "{fastq}.unsorted_readgroup.insert_size_histogram.pdf", fastq = config["input_basenames"]),
        files["insert_size_metrics"] = expand(Path(config['working_dir']) / "qc" / "unsorted_RG_quality" / "{fastq}.unsorted_readgroup.insert_size_metrics", fastq = config["input_basenames"]),
        files["quality_by_cycle_pdf"] = expand(Path(config['working_dir']) / "qc" / "unsorted_RG_quality" / "{fastq}.unsorted_readgroup.quality_by_cycle.pdf", fastq = config["input_basenames"]),
        files["quality_by_cycle_metrics"] = expand(Path(config['working_dir']) / "qc" / "unsorted_RG_quality" / "{fastq}.unsorted_readgroup.quality_by_cycle_metrics", fastq = config["input_basenames"]),
        files["quality_distribution_pdf"] = expand(Path(config['working_dir']) / "qc" / "unsorted_RG_quality" / "{fastq}.unsorted_readgroup.quality_distribution.pdf", fastq = config["input_basenames"]),
        files["quality_distribution_metrics"] = expand(Path(config['working_dir']) / "qc" / "unsorted_RG_quality" / "{fastq}.unsorted_readgroup.quality_distribution_metrics", fastq = config["input_basenames"]),

        files["alignment_summary_metrics"] = Path(config['working_dir']) / "qc" / "RG_quality" / f"{config['sample']}.readgroup.alignment_summary_metrics",
        files["gc_bias_detail_metrics"] = Path(config['working_dir']) / "qc" / "RG_quality" / f"{config['sample']}.readgroup.gc_bias.detail_metrics",
        files["gc_bias_pdf"] = Path(config['working_dir']) / "qc" / "RG_quality" / f"{config['sample']}.readgroup.gc_bias.pdf",
        files["gc_bias_summary_metrics"] = Path(config['working_dir']) / "qc" / "RG_quality" / f"{config['sample']}.readgroup.gc_bias.summary_metrics",

        files["mark_duplicates_metrics"] = Path(config['working_dir']) / 'qc' / 'mark_duplicates' / "duplicate_metrics.txt.io",

        files["alignment_summary_metrics_agg"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.alignment_summary_metrics",
        files["bait_bias_detail_metrics"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.bait_bias_detail_metrics",
        files["bait_bias_summary_metrics"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.bait_bias_summary_metrics",
        files["gc_bias_detail_metrics_agg"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.gc_bias.detail_metrics",
        files["gc_bias_pdf_agg"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.gc_bias.pdf",
        files["gc_bias_summary_metrics_agg"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.gc_bias.summary_metrics",
        files["insert_size_histogram_pdf_agg"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.insert_size_histogram.pdf",
        files["insert_size_metrics_agg"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.insert_size_metrics",
        files["pre_adapter_detail_metrics"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.pre_adapter_detail_metrics",
        files["pre_adapter_summary_metrics"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.pre_adapter_summary_metrics",
        files["quality_distribution_pdf_agg"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.quality_distribution.pdf",
        files["quality_distribution_metrics_agg"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.quality_distribution_metrics",
        files["error_summary_metrics"] = Path(config['working_dir']) / "qc" / "aggregation_metrics" / f"{config['sample']}.agg.error_summary_metrics",

        files["TXT_metrics_checksum"] = Path(config['working_dir']) / "qc" / "RG_checksum" / f"{config['sample']}.bam.read_group_md5",

        files["TXT_metrics_WGS"] = Path(config['working_dir']) / "qc" / "wgs_metrics" / f"{config['sample']}.wgs.metrics.txt",
        files["TXT_metrics_rawWGS"] = Path(config['working_dir']) / "qc" / "wgs_metrics" / f"{config['sample']}.raw.wgs.metrics.txt",

        files["TXT_metrics_CRAM"] = Path(config['working_dir']) / "qc" / "bamtocram" / "cram_validation_report.io",

        files["TXT_metrics_varCalling"] = Path(config['working_dir']) / "qc" / "variant_calling_metrics" / f"{config['sample']}.variant_calling_metrics.io",

        files["QC_summary"] = Path(config['working_dir']) / "qc" / "QC_summary.txt"
...
rule move_qc:
    """
    Move all QC output files to final output QC directory. If the "--no-qc" flag is used, only the mark duplicates qc
    file will be generated (and thus moved)
    """
    input:
         unpack(qc_output_files)
    output:
        QC_done = Path(config['final_output_dir']) / 'qc' / 'qc_done.txt'
    params:
    ...
    run:
        input_files_local = qc_output_files(wildcards)

        outdir = Path(config['final_output_dir']) / 'qc' / 'mark_duplicates'
        outdir.mkdir(parents=True, exist_ok=True)

        if config["no_qc"]:
            logging.debug(SnakemakeUtils.load_object(input_files_local['mark_duplicates_metrics'][0]))
            SnakemakeUtils.load_object(input_files_local['mark_duplicates_metrics'][0])[0].path.link_to(params.mark_duplicates_metrics)
        else:
