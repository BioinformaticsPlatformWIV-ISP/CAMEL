...
class HybridAssemblyReporter(Tool):
    ...
    def _check_input(self) -> None:
        ...
        :return: None
        """
        if any(key for key in ['quast', 'vc', 'sniffles', 'mapping', 'commands'] not in self._input_informs.keys()):
            raise InvalidInputSpecificationError('Require QUAST, Variant calling, Sniffles, '
                                                 'Mapping and commands input as informs.')
        super()._check_input()
    ...
    def _execute_tool(self) -> None:
        ...
        :return: None
        """
        section = HtmlReportSection(HybridAssemblyReporter.TITLE, subtitle='version 0.1')
        self.__add_quast_table(section)

        for tool in self._input_informs.keys():
            header, data = self.__parse_input_file(tool)
    ...
    def __add_quast_table(self, section: HtmlReportSection):
        div_sect = HtmlElement('div', attributes=[('class', 'border_bottom')])
        div_sect.add_header('QUAST statistics', 3)
        div = HtmlExpandableDiv('quast_statistics', 'quast')
        quast_table = pickle.load(open(input.informs_quast, 'rb'))
        div.add_table(quast_table, column_names=['Assembly step', 'N50', 'No of contigs', 'Total length'],
    ...
    def __add_vc_table(self, section: HtmlReportSection):
        div_sect = HtmlElement('div', attributes=[('class', 'border_bottom')])
        div_sect.add_header('QUAST statistics', 3)
        div = HtmlExpandableDiv('quast_statistics', 'quast')
        quast_table = pickle.load(open(self._input_informs['quast'], 'rb')) # not sure - need to check this
        div.add_table(quast_table, column_names=['Assembly step', 'N50', 'No of contigs', 'Total length'], table_attributes=[('class', 'information')])
        div_sect.add_html_object(div)

    def __parse_input_file(self, name_of_tool: str) -> Tuple[Dict[str, List[str]], Dict[str, List[List[str]]]]:
        """
