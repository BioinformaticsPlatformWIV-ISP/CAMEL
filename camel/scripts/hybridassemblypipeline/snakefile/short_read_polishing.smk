...
rule bwa_index:
    ...
    input:
        FASTA_REF = rules.polca_polishing.output.FASTA
    output:
        INDEX_GENOME_PREFIX = Path(config['working_dir']) / 'polishing' / 'polca' / 'genome_prefix.io'
    params:
        running_dir = Path(config['working_dir']) / 'polishing' / 'polca'
    run:
...
rule read_mapping:
    ...
    input:
        ...
        INDEX_GENOME_PREFIX_SAMTOOLS=rules.samtools_index_short.output.INDEX_GENOME_PREFIX,
...
rule polypolish_polishing:
    """
    Then polishing with polypolish.
    """
    input:
