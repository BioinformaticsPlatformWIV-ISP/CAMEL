...
| Coverage against assembled contigs     | 20x / 50x (ONT)       | 10x / 30x (ONT)    | Coverage of the reads mapped to the assembly (determined by QUAST)                                                                                                                                                              |
...
| NanoPlot: Median read length           | 3000                  | 2000               | Checks if the median read length of the trimmed reads is below a threshold compared to the mode length of the raw input reads (251).                                                                                            |
| NanoPlot: Median read quality          | 16                    | 14                 | Checks if the median read length of the trimmed reads is below a threshold compared to the mode length of the raw input reads (251).                                                                                            |
...
updated version of blast (`blast 2.14.0`). Alternative detection using `kma 1.4.12a` is available by 
