...
rule serotyping_seqsero2_run_fasta:
    ...
    input:
        FASTA = Path(config['working_dir']) / assembly.OUTPUT_ASSEMBLY_FASTA
    output:
        TXT = Path(config['working_dir']) / 'serotyping' / 'serotyping_seqsero2_Kmer' / 'SeqSero2_result.io',
        INFORMS = Path(config['working_dir']) / 'serotyping' / 'serotyping_seqsero2_Kmer' / 'informs.io'
    params:
        mode = 'Kmer',
        running_dir = lambda wildcards: Path(config['working_dir']) / 'serotyping' / f'serotyping_seqsero2_Kmer',
