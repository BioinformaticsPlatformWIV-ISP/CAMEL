...
from camel.resources.snakefile.read_trimming import OUTPUT_READ_TRIMMING_READS_PE, OUTPUT_READ_TRIMMING_SUMMARY, \
    OUTPUT_READ_TRIMMING_INFORMS
from camel.resources.snakefile.read_trimming_iontorrent import get_read_trimming_report, OUTPUT_TRIMMING_IT_READS
...
from camel.scripts.shigellapipeline import SNAKEFILE_SUBSPECIES, SNAKEFILE_FLEXNERI_TYPE
from camel.scripts.shigellapipeline.snakefile.flexneritype import OUTPUT_FLEXNERI_SUMMARY, OUTPUT_FLEXNERI_REPORT_EMPTY, \
    OUTPUT_FLEXNERI_REPORT
from camel.scripts.shigellapipeline.snakefile.subspecies_identification import OUTPUT_SUBSPECIES_REPORT, \
    OUTPUT_SPECIES_REPORT, OUTPUT_SUBSPECIES_SUMMARY, OUTPUT_SPECIES_REPORT_EMPTY, OUTPUT_SUBSPECIES_REPORT_EMPTY
...
include: SNAKEFILE_SUBSPECIES
include: SNAKEFILE_FLEXNERI_TYPE
...
rule Combine_reports:
    ...
    input:
        ...
        report_kraken=os.path.join(config['working_dir'], OUTPUT_CONTAMINATION_CHECK_REPORT if 'kraken' in config['analyses'] else OUTPUT_CONTAMINATION_CHECK_REPORT_EMPTY),
        ...
        report_pointfinder=os.path.join(config['working_dir'], OUTPUT_POINTFINDER_REPORT if 'pointfinder' in config['analyses'] else OUTPUT_POINTFINDER_REPORT_EMPTY),
        ...
        report_species=os.path.join(config['working_dir'], OUTPUT_SPECIES_REPORT if 'identification' in config['analyses'] else OUTPUT_SPECIES_REPORT_EMPTY),
        report_subspecies=os.path.join(config['working_dir'], OUTPUT_SUBSPECIES_REPORT if 'identification' in config['analyses'] else OUTPUT_SUBSPECIES_REPORT_EMPTY),
        report_flexneri=os.path.join(config['working_dir'], OUTPUT_FLEXNERI_REPORT if 'identification' in config['analyses'] else OUTPUT_FLEXNERI_REPORT_EMPTY),
        report_subspecies_db=os.path.join(config['working_dir'], 'subspecies_identification', 'report', 'html-db.io'),
    ...
    run:
        ...
        report_structure = [
            ...
            ('Subspecies identification', 'subspecies', [input.report_species, input.report_subspecies,
                                                         input.report_flexneri, input.report_subspecies_db]),
...
rule Create_commands_section:
    input:
        INFORMS_trimmomatic = os.path.join(config['working_dir'], OUTPUT_READ_TRIMMING_INFORMS),
    ...
    output:
        VAL_HTML = os.path.join(config['working_dir'], 'report', 'html-commands.io')
    params:
