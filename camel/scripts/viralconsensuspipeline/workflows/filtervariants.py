...
class FilterVariants(object):
    ...
    Supports certain filters for VCF files generated using Clair3 or bcftools.
    """
    ...
    def __apply_filter(self, path_vcf_in: Path, caller: str, filter_key: str, filter_value: str) -> Path:
        ...
        bcftools_filter.add_input_files({'VCF': [ToolIOFile(path_vcf_in)]})
        bcftools_filter.update_parameters(
            soft_filter=filter_key, exclude=f'"{expression}"', output_filename=str(path_out), output_type='v')
        bcftools_filter.run(self._dir)
        self._informs.append(bcftools_filter.informs)
        return path_out
