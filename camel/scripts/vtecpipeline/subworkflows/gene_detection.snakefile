...
DATABASES = {
    ...
    'virulence': [
        GeneDatabase('VirulenceFinder',
                     '/data/sequence_db/VirulenceFinder-Ecoli',
                     '/data/srst2/gene_db/VirulenceFinder-Ecoli-clustered_80',
                     ('Protein function', 'protein_function',), 90),
        GeneDatabase('Shiga-Toxin_genes',
                     '/data/sequence_db/VirulenceFinder-Shiga-toxin_genes',
                     '/data/srst2/gene_db/VirulenceFinder-Shiga-toxin_genes-clustered_80',
                     ('Protein function', 'protein_function',), 90)
    ],
    'serotype': [
        GeneDatabase('H_type',
                     '/data/sequence_db/SerotypeFinder-H_type',
                     '/data/srst2/gene_db/SerotypeFinder-H_type-clustered_80',
                     ('Predicted serotype', 'predicted_serotype',), 85),
        GeneDatabase('O_type',
                     '/data/sequence_db/SerotypeFinder-O_type',
                     '/data/srst2/gene_db/SerotypeFinder-O_type-clustered_80',
                     ('Predicted serotype', 'predicted_serotype',), 85),
    ],
    'plasmid': [
        GeneDatabase('Enterobacteriaceae',
                     '/data/sequence_db/PlasmidFinder-enterobacteriaceae',
                     '/data/srst2/gene_db/PlasmidFinder-enterobacteriaceae-clustered_80',
                     ['Notes', 'notes'], 95)
    ]
}
...
rule get_clustered_db:
    ...
    run:
        from camel.app.tools.pipelines.gene_detection.dbmanagerclustered import DBManagerClustered
...
rule gene_detection_get_hits:
    ...
    """
    input:
        hits_blast=os.path.join(__WORKING_DIR, 'gene_detection', '{db}', 'alignment_extraction', 'blast-hits.io') if config['detection_method'] == 'fast' else [],
        hits_srst2=os.path.join(__WORKING_DIR, 'gene_detection', '{db}', 'srst2', 'srst2-hits.io') if config['detection_method'] == 'normal' else [],
        tsv_blast=os.path.join(__WORKING_DIR, 'gene_detection', '{db}', 'tsv-filtered.io') if config['detection_method'] == 'fast' else [],
        tsv_srst2=os.path.join(__WORKING_DIR, 'gene_detection', '{db}', 'tsv-srst2.io') if config['detection_method'] == 'normal' else []
    output:
...
rule combine_gene_detection_reports:
    ...
    run:
        ...
        for title, input_html in [('Resistance Characterization', input.HTML_Res),
                                  ...
                                  ('Serotype Determination', input.HTML_Ser)]:
            if len(input_html) > 0:
                ...
                if title == 'Serotype Determination':
                    section_serotype = HtmlReportSection(None)
                    serotype = SnakemakeUtils.load_object(input.VAL_serotype)[0].value
                    section_serotype.add_paragraph("Predicted serotype: {}".format(serotype))
                    gene_detection_module.add_html_object(section_serotype)
...
rule get_serotype:
    ...
    run:
        from camel.app.tools.pipelines.gene_detection.serotypedetectorecoli import SerotypeDetectorEcoli
