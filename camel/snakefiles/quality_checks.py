...
QC_CHECKS_BY_KEY = {
    ...
    for qc in [
        ...
        QCCheck(
            ...
            explanation="Percent of loci detected for the '{}' typing scheme."
        ),
        QCCheck(
...
def get_qc_checks(
    input_type: str,
    skipped_checks: list[str] = None,
    forced_checks: Optional[list[str]] = None,
) -> list[Path]:
    """
    ...
    for key, qc_check in QC_CHECKS_BY_KEY.items():
        ...
        if '{ori}' in key:
            paths.extend(
                [
                    Path('quality_checks', f'{key.format(ori=ori)}.json')
                    for ori in ('fwd', 'rev')
                ]
            )
        else:
