...
class TestNcbiHumanReadScrubber(CamelTestSuite):
    ...
    def test_scrubber(self) -> None:
        ...
        scrubber.add_input_files({'FASTQ_SINGLE_GUNZIP':
                                  [ToolIOFile(TestNcbiHumanReadScrubber.test_file_dir / 'Myco-DRR041783-ds_1_subset.fastq')]})
        ...
        self.assertGreater((TestNcbiHumanReadScrubber.test_file_dir / 'Myco-DRR041783-ds_1_subset.fastq').stat().st_size,
                           (self.running_dir / 'test_scrubber_output.fastq').stat().st_size)
