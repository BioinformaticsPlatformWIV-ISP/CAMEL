...
"""
OUTPUT_TYPING_REPORT = os.path.join(__WORKING_DIR, 'typing', 'html.io')
...
rule extract_locus_set_info:
    ...
    output:
        INFORMS=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'informs-locus_set.io')
    params:
        running_dir=os.path.join(__WORKING_DIR, 'typing-{scheme}')
    run:
...
rule extract_locus_info:
    ...
    output:
        FASTA=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'fasta-{locus}.io'),
        INFORMS=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'informs-{locus}.io')
    params:
        running_dir=os.path.join(__WORKING_DIR, 'typing-{scheme}')
    run:
...
rule pickle_dump_sequence_type_definitions:
    ...
    output:
        os.path.join(__WORKING_DIR, 'typing-{scheme}', 'tsv-profiles.io')
    run:
        SnakemakeUtils.dump_object([ToolIOFile(os.path.join(input[0], 'profiles.tsv'))], output[0])

rule blast_allele_detection:
    """
    Allele detection using blastn.
    """
    input:
        FASTA=OUTPUT_ASSEMBLY_FASTA,
        DB_BLAST=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'fasta-{locus}.io'),
        INFORMS_locus=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'informs-{locus}.io')
    output:
        VAL_Hit=os.path.join(__WORKING_DIR, 'typing-{scheme}', '{locus}', 'hit-blast.io')
    params:
        working_dir=os.path.join(__WORKING_DIR, 'typing-{scheme}', '{locus}')
    threads: 1
    run:
...
rule srst2_allele_detection:
    ...
    input:
        ...
        INFORMS_locus=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'informs-{locus}.io')
    output:
        VAL_Hit=os.path.join(__WORKING_DIR, 'typing-{scheme}', '{locus}', 'hit-srst2.io')
    params:
        running_dir=os.path.join(__WORKING_DIR, 'typing-{scheme}', '{locus}')
    threads: 4
    run:
...
rule combine_hits:
    ...
    """
    input:
        hits_blast=lambda wildcards: expand(os.path.join(__WORKING_DIR, 'typing-{scheme}', '{locus}', 'hit-blast.io'),
                                            locus=LOCI.get(wildcards.scheme),
                                            scheme=wildcards.scheme) if config['detection_method'] == 'fast' else [],
        hits_srst2=lambda wildcards: expand(os.path.join(__WORKING_DIR, 'typing-{scheme}', '{locus}', 'hit-srst2.io'),
                                            locus=LOCI.get(wildcards.scheme),
                                            scheme=wildcards.scheme) if config['detection_method'] == 'normal' else []
    output:
        os.path.join(__WORKING_DIR, 'typing-{scheme}', 'all-hits.io')
    run:
        list_of_hits = []
...
rule combine_loci:
    ...
    """
    input:
        hits=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'all-hits.io'),
        INFORMS_Loci=lambda wildcards: expand(os.path.join(__WORKING_DIR, 'typing-{scheme}', 'informs-{locus}.io'),
                                              locus=LOCI.get(wildcards.scheme), scheme=wildcards.scheme)
    output:
        TSV=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'tsv-combined.io')
    params:
        running_dir=os.path.join(__WORKING_DIR, 'typing-{scheme}'),
...
rule detect_sequence_type:
    ...
    """
    input:
        VAL_Hits=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'all-hits.io'),
        TSV=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'tsv-profiles.io')
    output:
        INFORMS=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'informs-st.io')
    params:
        running_dir=os.path.join(__WORKING_DIR, 'typing-{scheme}')
    run:
...
rule create_report:
    ...
    input:
        TSV=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'tsv-combined.io'),
        INFORMS_Scheme=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'informs-locus_set.io'),
        INFORMS_ST=lambda wildcards: os.path.join(__WORKING_DIR, 'typing-{}'.format(wildcards.scheme), 'informs-st.io') if has_profiles(wildcards.scheme) else [],
        VAL_Hits=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'all-hits.io'),
    output:
        VAL_HTML=os.path.join(__WORKING_DIR, 'typing-{scheme}', 'html-report.io')
    params:
        running_dir=os.path.join(__WORKING_DIR, 'typing-{scheme}')
    run:
...
rule combine_sequence_typing_reports:
    ...
    """
    input:
        VAL_HTML=expand(os.path.join(__WORKING_DIR, 'typing-{scheme}', 'html-report.io'), scheme=SCHEMES)
    output:
        VAL_HTML=os.path.join(__WORKING_DIR, 'typing', 'html.io')
    params:
        running_dir=os.path.join(__WORKING_DIR, 'typing'),
